...
def get_all_experiments()
  ...
  selector = experiment_srv.module::ExperimentSelector.new
  ...
  if response and response.rval and response.rval.entries
    experiments = response.rval.entries
    experiments.each do |experiment|
      experiment_stats = experiment.experimentSummaryStats
      puts "Experiment with name \"#{experiment.name}\", " +
           "id #{experiment.id} and control id " +
           "#{experiment.controlId} was found and it includes " +
           "#{experiment_stats.adGroupsCount} ad group(s) and " +
           "#{experiment_stats.adGroupCriteriaCount} criteria.\n"
    end
  else
